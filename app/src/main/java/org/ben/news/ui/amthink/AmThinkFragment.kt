package org.ben.news.ui.amthink
...
class AmThinkFragment : Fragment(), StoryListener, MenuProvider {

    companion object {
        fun newInstance() = AmThinkFragment()
    }

    private var _fragBinding: FragmentAmThinkBinding? = null
    private val fragBinding get() = _fragBinding!!
    lateinit var loader: AlertDialog
    private val loggedInViewModel: LoggedInViewModel by activityViewModels()
    private val amViewModel: AmThinkViewModel by activityViewModels()
    ...
    override fun onCreateView(
        ...
        showLoader(loader, "")
        _fragBinding = FragmentAmThinkBinding.inflate(inflater, container, false)
        ...
        fragBinding.recyclerViewAm.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            var y = 0
        ...
        fab!!.setOnClickListener {
            fragBinding.recyclerViewAm.smoothScrollToPosition(0)
            bot!!.visibility = View.VISIBLE
        }

        amViewModel.observableAmList.observe(viewLifecycleOwner) { story ->
            story?.let {
                render(story as ArrayList<StoryModel>)
                checkSwipeRefresh()
            }
            hideLoader(loader)
            if (fragBinding.recyclerViewAm.adapter!!.itemCount == 0 && searching != null) {
                val st = ArrayList<StoryModel>()
                st.add(StoryModel(title = "1"))
                fragBinding.recyclerViewAm.adapter = EmptyAdapter(st, this)
                state?.let { fragBinding.recyclerViewAm.layoutManager?.onRestoreInstanceState(it) }
            } else if (fragBinding.recyclerViewAm.adapter!!.itemCount == 0) {
                fragBinding.creepy.visibility = View.VISIBLE
            }
            if (fragBinding.recyclerViewAm.adapter!!.itemCount > 0) {
    ...
    private fun setSwipeRefresh() {
        fragBinding.swipe.setOnRefreshListener {
            ...
            state = fragBinding.recyclerViewAm.layoutManager?.onSaveInstanceState()
            amViewModel.load(day)
        }
    }
    ...
    private fun render(storyList: ArrayList<StoryModel>) {
        fragBinding.recyclerViewAm.adapter = StoryAdapter(storyList, this)
        state?.let { fragBinding.recyclerViewAm.layoutManager?.onRestoreInstanceState(it) }
    }
