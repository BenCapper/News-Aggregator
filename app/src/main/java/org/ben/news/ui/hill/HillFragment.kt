...
import org.ben.news.databinding.FragmentHillBinding
...
class HillFragment : Fragment(), StoryListener, MenuProvider {

    companion object {
        fun newInstance() = HillFragment()
    }

    private var _fragBinding: FragmentHillBinding? = null
    private val fragBinding get() = _fragBinding!!
    lateinit var loader: AlertDialog
    private val loggedInViewModel: LoggedInViewModel by activityViewModels()
    private val hillViewModel: HillViewModel by activityViewModels()
    ...
    override fun onCreateView(
        ...
        showLoader(loader, "")
        _fragBinding = FragmentHillBinding.inflate(inflater, container, false)
        val root = fragBinding.root

        fragBinding.recyclerViewHill.layoutManager = activity?.let { LinearLayoutManager(it) }
        activity?.findViewById<ImageView>(R.id.toolimg)?.setImageResource(R.drawable.hill)
        MobileAds.initialize(this.requireContext()) {}
        val fab = activity?.findViewById<FloatingActionButton>(R.id.fab)
        val bot = activity?.findViewById<BottomNavigationView>(R.id.bottom_nav)
        fab?.visibility = View.INVISIBLE
        fragBinding.recyclerViewHill.addOnScrollListener(object :
        ...
        fab!!.setOnClickListener {
            fragBinding.recyclerViewHill.smoothScrollToPosition(0)
            bot!!.visibility = View.VISIBLE
        }
        hillViewModel.observableHillList.observe(viewLifecycleOwner) { story ->
            story?.let {
                render(story as ArrayList<StoryModel>)
                checkSwipeRefresh()
            }
            hideLoader(loader)
            if (fragBinding.recyclerViewHill.adapter!!.itemCount == 0 && searching != null) {
                val st = ArrayList<StoryModel>()
                st.add(StoryModel(title = "1"))
                fragBinding.recyclerViewHill.adapter = EmptyAdapter(st, this)
                state?.let { fragBinding.recyclerViewHill.layoutManager?.onRestoreInstanceState(it) }
            } else if (fragBinding.recyclerViewHill.adapter!!.itemCount == 0) {
                fragBinding.creepy.visibility = View.VISIBLE
            }
            if (fragBinding.recyclerViewHill.adapter!!.itemCount > 0) {
    ...
    private fun setSwipeRefresh() {
        fragBinding.swipe.setOnRefreshListener {
            ...
            state = fragBinding.recyclerViewHill.layoutManager?.onSaveInstanceState()
            hillViewModel.load(day)
        }
    }
    ...
    private fun render(storyList: ArrayList<StoryModel>) {
        fragBinding.recyclerViewHill.adapter = StoryAdapter(storyList, this)
        state?.let { fragBinding.recyclerViewHill.layoutManager?.onRestoreInstanceState(it) }
    }
