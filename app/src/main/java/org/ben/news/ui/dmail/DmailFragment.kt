...
import org.ben.news.databinding.FragmentDmailBinding
...
class DmailFragment : Fragment(), StoryListener, MenuProvider {

    companion object {
        fun newInstance() = DmailFragment()
    }

    private var _fragBinding: FragmentDmailBinding? = null
    private val fragBinding get() = _fragBinding!!
    lateinit var loader: AlertDialog
    private val loggedInViewModel: LoggedInViewModel by activityViewModels()
    private val dmailViewModel: DmailViewModel by activityViewModels()
    ...
    override fun onCreateView(
        ...
        showLoader(loader, "")
        _fragBinding = FragmentDmailBinding.inflate(inflater, container, false)
        ...
        fragBinding.recyclerViewDmail.addOnScrollListener(object :
        ...
        fab!!.setOnClickListener {
            fragBinding.recyclerViewDmail.smoothScrollToPosition(0)
            bot!!.visibility = View.VISIBLE
        }
        dmailViewModel.observableMailList.observe(viewLifecycleOwner) { story ->
            story?.let {
                render(story as ArrayList<StoryModel>)
                checkSwipeRefresh()
            }
            hideLoader(loader)
            if (fragBinding.recyclerViewDmail.adapter!!.itemCount == 0 && searching != null) {
                val st = ArrayList<StoryModel>()
                st.add(StoryModel(title = "1"))
                fragBinding.recyclerViewDmail.adapter = EmptyAdapter(st, this)
                state?.let { fragBinding.recyclerViewDmail.layoutManager?.onRestoreInstanceState(it) }
            } else if (fragBinding.recyclerViewDmail.adapter!!.itemCount == 0) {
                fragBinding.creepy.visibility = View.VISIBLE
            }
            if (fragBinding.recyclerViewDmail.adapter!!.itemCount > 0) {
    ...
    private fun setSwipeRefresh() {
        fragBinding.swipe.setOnRefreshListener {
            ...
            state = fragBinding.recyclerViewDmail.layoutManager?.onSaveInstanceState()
            dmailViewModel.load(day)
        }
    }
    ...
    private fun render(storyList: ArrayList<StoryModel>) {
        fragBinding.recyclerViewDmail.adapter = StoryAdapter(storyList, this)
        state?.let { fragBinding.recyclerViewDmail.layoutManager?.onRestoreInstanceState(it) }
    }
