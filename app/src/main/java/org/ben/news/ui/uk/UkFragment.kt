...
import org.ben.news.databinding.FragmentUkBinding
...
class UkFragment : Fragment(), StoryListener, MenuProvider {

    companion object {
        fun newInstance() = UkFragment()
    }

    private var _fragBinding: FragmentUkBinding? = null
    ...
    override fun onCreateView(
        ...
        fragBinding.recyclerViewUk.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            var y = 0
        ...
        ukViewModel.observableUkList.observe(viewLifecycleOwner) { story ->
            ...
            hideLoader(loader)
            if (fragBinding.recyclerViewUk.adapter!!.itemCount == 0 && searching != null) {
                val st = ArrayList<StoryModel>()
                st.add(StoryModel(title = "1"))
                fragBinding.recyclerViewUk.adapter = EmptyAdapter(st, this)
                state?.let { fragBinding.recyclerViewUk.layoutManager?.onRestoreInstanceState(it) }
            } else if (fragBinding.recyclerViewUk.adapter!!.itemCount == 0) {
