...
class NprFragment : Fragment(), StoryListener, MenuProvider {
    ...
    override fun onCreateView(
        ...
        activity?.findViewById<ImageView>(R.id.toolimg)?.setImageResource(R.drawable.npr)
        MobileAds.initialize(this.requireContext()) {}
        val fab = activity?.findViewById<FloatingActionButton>(R.id.fab)
        val bot = activity?.findViewById<BottomNavigationView>(R.id.bottom_nav)
        fab?.visibility = View.INVISIBLE
        fragBinding.recyclerViewNpr.addOnScrollListener(object :
        ...
        fab!!.setOnClickListener {
            fragBinding.recyclerViewNpr.smoothScrollToPosition(0)
        ...
        nprViewModel.observableNprList.observe(viewLifecycleOwner) { story ->
            ...
            hideLoader(loader)
            if (fragBinding.recyclerViewNpr.adapter!!.itemCount == 0 && searching != null) {
                val st = ArrayList<StoryModel>()
                st.add(StoryModel(title = "1"))
                fragBinding.recyclerViewNpr.adapter = EmptyAdapter(st, this)
                state?.let { fragBinding.recyclerViewNpr.layoutManager?.onRestoreInstanceState(it) }
            } else if (fragBinding.recyclerViewNpr.adapter!!.itemCount == 0) {
                fragBinding.creepy.visibility = View.VISIBLE
            }
            if (fragBinding.recyclerViewNpr.adapter!!.itemCount > 0) {
