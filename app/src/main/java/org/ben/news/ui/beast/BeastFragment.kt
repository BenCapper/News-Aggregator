...
class BeastFragment : Fragment(), StoryListener, MenuProvider {
    ...
    override fun onCreateView(
        ...
        fab!!.setOnClickListener {
            fragBinding.recyclerViewBeast.smoothScrollToPosition(0)
            bot!!.visibility = View.VISIBLE
        }

        beastViewModel.observableBeastList.observe(viewLifecycleOwner) { story ->
            story?.let {
    ...
    private fun render(storyList: ArrayList<StoryModel>) {
        fragBinding.recyclerViewBeast.adapter = StoryAdapter(storyList, this)
        state?.let { fragBinding.recyclerViewBeast.layoutManager?.onRestoreInstanceState(it) }
    }
