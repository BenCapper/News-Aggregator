...
import org.ben.news.databinding.FragmentVoxBinding
...
class VoxFragment : Fragment(), StoryListener, MenuProvider {

    companion object {
        fun newInstance() = VoxFragment()
    }

    private var _fragBinding: FragmentVoxBinding? = null
    ...
    override fun onCreateView(
        ...
        fragBinding.recyclerViewVox.layoutManager = activity?.let { LinearLayoutManager(it) }
        activity?.findViewById<ImageView>(R.id.toolimg)?.setImageResource(R.drawable.vox)
        MobileAds.initialize(this.requireContext()) {}
        val fab = activity?.findViewById<FloatingActionButton>(R.id.fab)
        val bot = activity?.findViewById<BottomNavigationView>(R.id.bottom_nav)
        fab?.visibility = View.INVISIBLE
        fragBinding.recyclerViewVox.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            var y = 0
        ...
        voxViewModel.observableVoxList.observe(viewLifecycleOwner) { story ->
            ...
            hideLoader(loader)
            if (fragBinding.recyclerViewVox.adapter!!.itemCount == 0 && searching != null) {
                val st = ArrayList<StoryModel>()
                st.add(StoryModel(title = "1"))
                fragBinding.recyclerViewVox.adapter = EmptyAdapter(st, this)
                state?.let { fragBinding.recyclerViewVox.layoutManager?.onRestoreInstanceState(it) }
            } else if (fragBinding.recyclerViewVox.adapter!!.itemCount == 0) {
                fragBinding.creepy.visibility = View.VISIBLE
            }
            if (fragBinding.recyclerViewVox.adapter!!.itemCount > 0) {
