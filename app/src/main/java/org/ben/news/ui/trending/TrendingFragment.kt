...
import org.ben.news.databinding.FragmentTrendingBinding
...
class TrendingFragment : Fragment(), StoryListener, MenuProvider {

    companion object {
        fun newInstance() = TrendingFragment()
    }

    private var _fragBinding: FragmentTrendingBinding? = null
    ...
    override fun onCreateView(
        ...
        showLoader(loader, "")
        _fragBinding = FragmentTrendingBinding.inflate(inflater, container, false)
        val root = fragBinding.root

        fragBinding.recyclerViewTrending.layoutManager = activity?.let { LinearLayoutManager(it) }
        activity?.findViewById<ImageView>(R.id.toolimg)?.setImageResource(R.drawable.trending)
        MobileAds.initialize(this.requireContext()) {}
        val fab = activity?.findViewById<FloatingActionButton>(R.id.fab)
        val bot = activity?.findViewById<BottomNavigationView>(R.id.bottom_nav)
        fab?.visibility = View.INVISIBLE
        fragBinding.recyclerViewTrending.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            var y = 0
        ...
        fab!!.setOnClickListener {
            fragBinding.recyclerViewTrending.smoothScrollToPosition(0)
            bot!!.visibility = View.VISIBLE
        }

        trendingViewModel.observableTrendingList.observe(viewLifecycleOwner) { story ->
            story?.let {
                render(story as ArrayList<StoryModel>)
                checkSwipeRefresh()
            }
            hideLoader(loader)
            if (fragBinding.recyclerViewTrending.adapter!!.itemCount == 0 && searching != null) {
                val st = ArrayList<StoryModel>()
                st.add(StoryModel(title = "1"))
                fragBinding.recyclerViewTrending.adapter = EmptyAdapter(st, this)
                state?.let { fragBinding.recyclerViewTrending.layoutManager?.onRestoreInstanceState(it) }
            } else if (fragBinding.recyclerViewTrending.adapter!!.itemCount == 0) {
                fragBinding.creepy.visibility = View.VISIBLE
            }
            if (fragBinding.recyclerViewTrending.adapter!!.itemCount > 0) {
    ...
    private fun setSwipeRefresh() {
        fragBinding.swipe.setOnRefreshListener {
            ...
            state = fragBinding.recyclerViewTrending.layoutManager?.onSaveInstanceState()
            trendingViewModel.load(day)
        }
    }
    ...
    private fun render(storyList: ArrayList<StoryModel>) {
        fragBinding.recyclerViewTrending.adapter = StoryAdapter(storyList, this)
        state?.let { fragBinding.recyclerViewTrending.layoutManager?.onRestoreInstanceState(it) }
    }
