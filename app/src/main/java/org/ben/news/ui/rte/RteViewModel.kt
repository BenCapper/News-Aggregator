...
class RteViewModel : ViewModel() {
    private val rteList =
        MutableLiveData<List<StoryModel>>()

    val observableRteList: LiveData<List<StoryModel>>
        get() = rteList
    ...
    fun load() {
        ...
        try {
            list = getDates(5)
            StoryManager.findByOutlet(list,outlet,rteList)
            Timber.i("Load Success : ${rteList.value}")
        }
        catch (e: Exception) {
    ...
    fun search( term: String) {
        ...
        try {
            dates = getDates(5)
            StoryManager.searchByOutlet(dates,term,outlet,rteList)
