...
double_log_gb = ("/home/bencapper/src/News-Aggregator/"
                 "scripts/doublelog/double.log")
double_gb_ref = list()
...
for article in right_log:
    ...
    for article2 in left_log:
        ...
        if (
            similar(title1, title2) > 0.7
            and article["title"] not in match_ref
            and article2["title"] not in match_ref
        ):
...
for match in matches:
    ...
    if right_article["link"] == left_article["link"]:
        ...
        pushDoubleToDB(
            db_path,
            str(key),
            titleDeFormat(right_article["title"]),
            right_article["date"],
            right_article["link"],
            right_article["outlet"],
            right_article["storage_link"],
            titleDeFormat(left_article["title"]),
            left_article["date"],
            left_article["link"],
            left_article["outlet"],
