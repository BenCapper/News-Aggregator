...
for article in articles:
    try:
       ...
       image = str(article).split('data-article-image="/')[1].split('" data')[0]
       img_src = f"{imagepre}{image}"
       title = str(article).split('data-headline="')[1].split('" data')[0]
       title = titleFormat(title)
       img_title = imgTitleFormat(title)
       date = str(article).split('data-article-image="/')[1].split('" data')[0]
       ...
       if title not in ref_list and check is False:
           ...
           storage_link = f"https://firebasestorage.googleapis.com/v0/b/news-a3e22.appspot.com/o/DMail%2F{td}%2F{img_title}?alt=media&token={token}"
           data = {
                "title": title,
                "date": date,
                "link": url,
